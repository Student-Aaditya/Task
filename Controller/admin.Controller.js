const bcrypt=require("bcrypt");
const con=require("../Config/db.js");

const adminController={
    createManager:async (req,res)=>{
        const { name, username, password, role, created_by } = req.body;

    if (!name || !username || !password || !role) {
        return res.status(400).json({ error: "Name, username, password, and role are required" });
    }

    const hashedPassword = await bcrypt.hash(password, 10);

    const sql = "INSERT INTO users (name, username, password, role, created_by) VALUES (?, ?, ?, ?, ?)";

     con.query(sql, [name, username, hashedPassword, role, created_by || null], (err, result) => {
        if (err) {
            if (err.code === "ER_DUP_ENTRY") {
                return res.status(400).json({ error: "Username already exists" });
            }
            return res.status(500).json({ error: err.message });
        }

        res.status(201).json({
            message: "Manager Created successfully",
            userId: result.insertId
        });
    });
    },
    managerList:(req,res)=>{
        const sql="SELECT * FROM users WHERE role='manager'";
       con.query(sql, (err, result) => {
        if (err) {
            return res.status(500).json({ msg: "Database error", error: err.message });
        }

        if (result.length === 0) {
            return res.status(404).json({ msg: "No Manager data found" });
        }

        return res.status(200).json({ msg: "Manager data", adminData: result });
    });
    },
    deleteManager:(req,res)=>{
     const { id } = req.params;

    const sql = "DELETE FROM users WHERE id=? AND role='manager'";
    con.query(sql, [id], (err, result) => {
        if (err){
             return res.status(500).json({ error: err.message })
            };

        if (result.affectedRows==0) {
            return res.status(404).json({ msg: "Manager not found" })
            };

        return  res.status(200).json({ msg: "Manager deleted successfully" });
    });
    },
     createVendor:async (req,res)=>{
        const { name, username, password, role, created_by } = req.body;

    if (!name || !username || !password || !role) {
        return res.status(400).json({ error: "Name, username, password, and role are required" });
    }

    const hashedPassword = await bcrypt.hash(password, 10);

    const sql = "INSERT INTO users (name, username, password, role, created_by) VALUES (?, ?, ?, ?, ?)";

     con.query(sql, [name, username, hashedPassword, role, created_by || null], (err, result) => {
        if (err) {
            if (err.code === "ER_DUP_ENTRY") {
                return res.status(400).json({ error: "Username already exists" });
            }
            return res.status(500).json({ error: err.message });
        }

        res.status(201).json({
            message: "Vendor Created successfully",
            userId: result.insertId
        });
    });
    },
    vendorList:(req,res)=>{
        const sql="SELECT * FROM users WHERE role='vendor'";
       con.query(sql, (err, result) => {
        if (err) {
            return res.status(500).json({ msg: "Database error", error: err.message });
        }

        if (result.length === 0) {
            return res.status(404).json({ msg: "No Vendor data found" });
        }

        return res.status(200).json({ msg: "Vendor data", adminData: result });
    });
    },
    deleteVendor:(req,res)=>{
     const { id } = req.params;

    const sql = "DELETE FROM users WHERE id=? AND role='vendor'";
    con.query(sql, [id], (err, result) => {
        if (err){
             return res.status(500).json({ error: err.message })
            };

        if (result.affectedRows==0) {
            return res.status(404).json({ msg: "Vendor not found" })
            };

        return  res.status(200).json({ msg: "Vendor deleted successfully" });
    });
    }
    }

module.exports=adminController;